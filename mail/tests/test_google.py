...
class OAuthFlowTests(GoogleTestCase):
    ...
    def test_new_google_user_gets_own_executive_workspace(self):
        ...
        self.assertEqual(list(user.owned_workspace.memberships.values_list('user_id', 'role')), [(user.pk, Membership.Role.EXECUTIVE)])
        ...
        self.assertEqual(self.workspace.memberships.get(role=Membership.Role.ASSISTANT).user_id, self.assistant.pk)
