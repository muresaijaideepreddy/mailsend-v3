...
**Current status: release `bc94d4c` is the last deployed release; access is currently timing out from the user's connection and end-to-end acceptance remains incomplete.** The assistant-account and date-only update was applied at 00:47 UTC on September 21. After preparation and reload, health, login, About and Privacy all returned HTTP 200 with expected content at 00:50:22 UTC. Browser inspection confirmed the styled login page and executive-only Google guidance. Later checks timed out for both the app and PythonAnywhere's main site, before TLS/HTTP, while GitHub returned HTTP 200. Local DNS agreed with Google's public resolver. This establishes a connection failure from this network, not its cause or a global outage.

## Worker setup update ready; deployment pending
...
The 13-file source-only archive `tmp/worker-setup-release/MailSend-V3-Worker-Setup.zip` has SHA-256 `9f394deed1b087130e039d01851caaf5d84d34680c76330e1209bb5b50076eec`; the sibling `update_worker_setup.py` validates before/after hashes and backs up source before replacement. The PythonAnywhere file manager timed out during the deployment attempt. No source upload, host migration, reload or live account retirement has occurred for this update. After access is restored, upload both files, run the helper with `--check` then `--apply`, run the existing private-environment `prepare` command, reload, and verify the result. The ready package contains no credentials, database or media.
