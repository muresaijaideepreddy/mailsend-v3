...
| Send Current Messages | Executive-approved IDs/versions; send date today or earlier in the app timezone, regardless of optional planning time | PDF December example, local calendar boundaries, date-only drafts, stale/replayed approvals and partial-result tests |
...
| Google identity and executive Gmail send | OAuth state/PKCE/nonce, verified identity, encrypted executive tokens, identity-only assistant linking/login and Gmail API | Mocked OAuth/provider tests pass; live Google consent and delivery of this revision remain pending |
...
| Worker account management | Executive signup provisions an inactive initial worker; executive sets profile/email and password; users change own passwords | Provisioning, username display, role/tenant/password-strength and Google-linking tests |
...
Dates use `MAILSEND_TIME_ZONE` (America/Chicago by default). Planning time is optional and never schedules delivery. Assistants can use username/password or Google identity after explicit one-time linking. HTML/rich-text email editing is not specified and plain-text composition is used. CSV creates one personalized draft per row; it never sends messages during import.
